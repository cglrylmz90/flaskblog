...
class RegisterForm (Form):
    ...
    email = StringField("E-mail Adresi", validators=[validators.Email(message="Geçerli bir E-mail adresi giriniz")])
...
@app.route("/")
def index():
    articles = [{"id":1, "title":"deneme1", "content":"deneme 1 içerik"},
...
def login():
    ...
    if request.method=="POST":
        ...
        if result > 0 :
            data=cursor.fetchone() #tablonun üzerinde arama yapan kod
...
def update(id):
    if request.method=="GET":
        ...
        result=cursor.execute(sorgu,(session["username"],id))
        if result==0:
            flash("Böyle bir makale yok veya bu işlemeye yetkiniz yok","danger")
            return redirect(url_for("index"))
        else:
